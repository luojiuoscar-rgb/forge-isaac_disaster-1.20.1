...
1. Before any Wiki or project search, resolve identity from the user name by first checking Chinese Wiki names and aliases, then cross-checking the English Wiki page name and the Wiki original item ID. Keep these fields separate and label them explicitly: Wiki original item name, Wiki original ID, mod `ItemId`, registry ID, Java name, texture name, and localization key.
2. If the Chinese Wiki lookup is still ambiguous, or names conflict after cross-checking, ask in Chinese for the intended Wiki page and its Wiki original ID; show at most three candidates with their names, Wiki original IDs, and differences. Do not infer a mapping from a similar English technical name. For example, never automatically map the Chinese name "地狱契约" to `THE_PACT`; ask the user to confirm the Wiki page and Wiki original ID first.
...
- For every new `IAbilityEffect.applyEffect(...)`, decide and document its boolean contract during planning. `true` means the effect path completed safely, including a confirmed acceptable no-op such as an ineligible ordinary scenario, empty input, or an intentionally skipped derived operation. `false` means the effect cannot honor its contract because a required context key, event/entity/target type, configured nested effect, or safe execution condition is missing or invalid; reserve it for failure semantics and a possible fallback. Do not return `false` merely because a chance did not roll (handle chance in the trigger), and do not swallow unexpected exceptions by converting them to `false`. `IExecutableEffect.apply(...)` itself is `void`; the current `IAbilityEffect.apply(...)` fallback branch is unfinished, so do not assume that `false` currently invokes a fallback. If a new fallback mechanism is needed, propose it separately for approval.
...
- Obtain only the 32x32-pixel PNG Repentance icon from the English Wiki, verify both PNG format and dimensions, and save it under the confirmed texture name. When choosing from wiki image files, names with the `Collectible_` prefix are usually the correct item images. Use the same 32x32 PNG requirement for active-item icons if active-item support is added later. Report missing icons; request all non-image assets from the user. If downloading fails because of network errors, an unavailable Wiki, an invalid or expired link, an HTTP error, or an unexpected file format, record the affected item and the concrete failure reason and state it explicitly in the final response; never silently omit the texture.
...
- Report changed files, reused/new modules, images, pools, `runData`, failed or unavailable texture downloads with their reasons, and missing non-image assets in Chinese.
