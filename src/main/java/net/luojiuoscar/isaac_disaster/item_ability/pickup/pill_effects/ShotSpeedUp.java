...
public class ShotSpeedUp implements IPillEffect {
    ...
    public int getPillEffectId() {
        return PillEffectId.SHOT_SPEED_UP.getId();
    }

    @Override
    public void onUse(Player player){
        if (PlayerHelper.getPillQuality(player) < 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SHOT_SPEED_DOWN.getId()).onUse(player);
            return;
        }
    ...
    public void onUseH(Player player){
        if (PlayerHelper.getPillQuality(player) < 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SHOT_SPEED_DOWN.getId()).onUseH(player);
            return;
        }
    ...
    public void onUseSound(Player player) {
        player.playSound(ModSounds.SHOT_SPEED_UP.get(), 1.0f, 1.0f);
        player.playSound(ModSounds.PILL_USE_GOOD_WITH_EFFECT.get(), 1.0f, 1.0f);
    }

    @Override
    public void onUseSoundH(Player player) {
        player.playSound(ModSounds.SHOT_SPEED_UP_H.get(), 1.0f, 1.0f);
