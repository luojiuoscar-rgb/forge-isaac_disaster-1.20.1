...
public class Paralysis implements IPillEffect {
    ...
    public void onUse(ServerPlayer player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.PHEROMONES.getId()).onUse(player);
            return;
        }
    ...
    public void onUseH(ServerPlayer player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.PHEROMONES.getId()).onUseH(player);
            return;
        }
