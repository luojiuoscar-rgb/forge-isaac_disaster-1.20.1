...
public class SpeedDown implements IPillEffect {
    ...
    public int getPillEffectId() {
        return PillEffectId.SPEED_DOWN.getId();
    }

    @Override
    public void onUse(Player player, boolean withSFX){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SPEED_UP.getId()).onUse(player, true);
            return;
        }
    ...
    public void onUseH(Player player, boolean withSFX){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SPEED_UP.getId()).onUseH(player, true);
            return;
        }
    ...
    public void onUseSound(Player player) {
        player.playSound(ModSounds.SPEED_DOWN.get(), 1.0f, 1.0f);
        player.playSound(ModSounds.PILL_USE_BAD.get(), 1.0f, 1.0f);
    }

    @Override
    public void onUseSoundH(Player player) {
        player.playSound(ModSounds.SPEED_DOWN_H.get(), 1.0f, 1.0f);
