...
public class ShotSpeedDown implements IPillEffect {
    ...
    public int getPillEffectId() {
        return PillEffectId.SHOT_SPEED_DOWN.getId();
    }

    @Override
    public void onUse(Player player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SHOT_SPEED_UP.getId()).onUse(player);
            return;
        }
    ...
    public void onUseH(Player player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.SHOT_SPEED_UP.getId()).onUseH(player);
            return;
        }
    ...
    public void onUseEffect(Player player) {
        ...
        StatManager.modifyBulletSpeedAdder(player, -0.5, true);
    }
    ...
    public void onUseEffectH(Player player) {
        ...
        StatManager.modifyBulletSpeedAdder(player, -1, true);
    }

    @Override
    public void onUseSound(Player player) {
        player.playSound(ModSounds.SHOT_SPEED_DOWN.get(), 1.0f, 1.0f);
        player.playSound(ModSounds.PILL_USE_BAD.get(), 1.0f, 1.0f);
    }

    @Override
    public void onUseSoundH(Player player) {
        player.playSound(ModSounds.SHOT_SPEED_DOWN_H.get(), 1.0f, 1.0f);
    ...
    public String getDescriptionId(){
        if (ClientDataManager.getInstance().getPillQuality() > 0){
            return "pill.isaac_disaster.effect.shot_speed_up";
        }
        return "pill.isaac_disaster.effect.shot_speed_down";
    }
