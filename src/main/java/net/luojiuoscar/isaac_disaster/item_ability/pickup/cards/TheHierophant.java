...
public class TheHierophant implements ITarot {
    @Override
    public int getItemId() {
        return PickupId.THE_HIEROPHANT.getId();
    }

    @Override
    public void onUseEffect(ServerPlayer player, ItemStack stack, InteractionHand hand) {
        LootHelper.spawnItemViaLoot(player, player.position(), ModItems.SOUL_HEART.get(), 2);
    }

    @Override
    public void onUseEffectStronger(ServerPlayer player, ItemStack stack, InteractionHand hand) {
        LootHelper.spawnItemViaLoot(player, player.position(), ModItems.SOUL_HEART.get(), 3);
    }

    @Override
    public void onUseSound(Player player) {
        player.playNotifySound(SoundEvents.BOOK_PAGE_TURN, SoundSource.PLAYERS, 1.0f, 1.0f);
        player.playNotifySound(ModSounds.THE_HIEROPHANT.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    }
    ...
    public List<Component> getDescription() {
        ...
        // 基础效果
        description.add(Component.translatable("item.isaac_disaster.action.give_soul_heart", 2));
        ...
        if (ClientDataManager.getInstance().getCountFromId(ItemId.TAROT_CLOTH.getId()) > 0){
            description.add(Component.translatable("item.isaac_disaster.tarot_cloth").append(": ")
                    .append(Component.translatable("item.isaac_disaster.action.give_soul_heart", 3))
