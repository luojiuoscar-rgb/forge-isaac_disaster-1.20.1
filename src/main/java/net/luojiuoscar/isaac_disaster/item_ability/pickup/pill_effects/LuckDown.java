...
public class LuckDown implements IPillEffect {
    @Override
    public int getPillEffectId() {
        return PillEffectId.LUCK_DOWN.getId();
    }

    @Override
    public void onUse(Player player, boolean withSFX){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.LUCK_UP.getId()).onUse(player, true);
            return;
        }
    ...
    public void onUseH(Player player, boolean withSFX){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.LUCK_UP.getId()).onUseH(player, true);
            return;
        }
    ...
    public void onUseSound(Player player) {
        player.playSound(ModSounds.LUCK_DOWN.get(), 1.0f, 1.0f);
        player.playSound(ModSounds.PILL_USE_BAD.get(), 1.0f, 1.0f);
    }

    @Override
    public void onUseSoundH(Player player) {
        player.playSound(ModSounds.LUCK_DOWN_H.get(), 1.0f, 1.0f);
    ...
    public String getDescriptionId(){
        if (ClientDataManager.getInstance().getPillQuality() > 0){
            return "pill.isaac_disaster.effect.luck_up";
        }
        return "pill.isaac_disaster.effect.luck_down";
    }
