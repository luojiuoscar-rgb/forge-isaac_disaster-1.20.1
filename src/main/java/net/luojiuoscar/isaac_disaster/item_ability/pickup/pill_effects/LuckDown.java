...
public class LuckDown implements IPillEffect {
    ...
    public void onUse(Player player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.LUCK_UP.getId()).onUse(player);
            return;
        }
    ...
    public void onUseH(Player player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.LUCK_UP.getId()).onUseH(player);
            return;
        }
