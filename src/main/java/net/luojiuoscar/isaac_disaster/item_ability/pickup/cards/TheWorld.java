...
public class TheWorld implements ITarot {
    @Override
    public int getItemId() {
        return PickupId.THE_WORLD.getId();
    }

    @Override
    public void onUseEffect(ServerPlayer player, ItemStack stack, InteractionHand hand) {
        player.addEffect(new MobEffectInstance(ModEffects.THE_WORLD.get(), 200));
    }

    @Override
    public void onUseEffectStronger(ServerPlayer player, ItemStack stack, InteractionHand hand) {
        player.addEffect(new MobEffectInstance(ModEffects.THE_WORLD.get(), 400));
    }

    @Override
    public void onUseSound(Player player) {
        player.playNotifySound(SoundEvents.BOOK_PAGE_TURN, SoundSource.PLAYERS, 1.0f, 1.0f);
        player.playNotifySound(ModSounds.THE_WORLD.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    ...
    public List<Component> getDescription() {
        ...
        // 基础效果
        description.add(Component.translatable("item.isaac_disaster.the_world.lore.1"));
