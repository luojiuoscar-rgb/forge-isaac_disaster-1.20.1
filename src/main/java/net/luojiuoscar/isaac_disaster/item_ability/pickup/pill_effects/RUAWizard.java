...
public class RUAWizard implements IPillEffect {
    ...
    public void onUse(ServerPlayer player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.POWER_PILL.getId()).onUse(player);
            return;
        }
    ...
    public void onUseH(ServerPlayer player){
        if (PlayerHelper.getPillQuality(player) > 0){
            PillEffectManager.getInstance().getEffectFromEffectId(PillEffectId.POWER_PILL.getId()).onUseH(player);
            return;
        }
