package net.luojiuoscar.isaac_disaster.manager;

public class EffectNameManager {
    public static final String POISON = "poison";
    public static final String POWER_OF_BELIAL = "power_of_belial";
    public static final String DIZZINESS = "dizziness";
    public static final String TRANSCENDENCE = "transcendence";
    public static final String INVINCIBLE = "invincible";
    public static final String FRAILTY = "frailty";
    public static final String NECRONMICON_SHIELD = "necronmicon_shield";
    public static final String HOLY_SHIELD = "holy_shield";
    public static final String LACRIMAL_HYPOSECRETION = "lacrimal_hyposecretion";
    public static final String X_RAY_VISION = "x_ray_vision";
    public static final String CHARM = "charm";
    public static final String VULNERABLE = "vulnerable";
    public static final String PANIC = "panic";
    public static final String PAC_MAN = "pac_man";
    public static final String RAMPAGE = "rampage";
    public static final String FRAGILE_HEART = "fragile_heart";
    public static final String TELEPATHY = "telepathy";
    public static final String BABYLON = "babylon";
    public static final String SOUL_STATE = "soul_state";
    public static final String THE_WORLD = "the_world";






}

