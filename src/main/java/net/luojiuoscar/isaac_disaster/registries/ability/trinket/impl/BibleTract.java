...
public class BibleTract extends TrinketAbility {
    public BibleTract(int id, int level) {
        super(id, level);
    }
    ...
    public void onEquipped(LivingEntity entity, TrinketAbilityContext ctx) {
        if (ctx.isEnchanted) {
            StatManager.addTriggerModule(entity, ModTriggerModule.BIBLE_TRACT.getId(), 2);
        }
        else {
            StatManager.addTriggerModule(entity, ModTriggerModule.BIBLE_TRACT.getId(), 1);
        }
    ...
    public List<Component> getDesc(@javax.annotation.Nullable ItemStack stack) {
        return List.of(
                Component.translatable("item.isaac_disaster.bible_tract.lore.1")
        );
    }
