...
public class YumHeart extends ActiveAbility {
    private static final CompositeTrigger TRIGGER = new CompositeTrigger(List.of(
            new SimpleTrigger(ModTriggerTypes.EMTPY, ModAbilityEffects.HEAL)
    ));

    public YumHeart(int id, int level) {
        super(TRIGGER, id, level);
    }

    @Override
    public void triggerSFX(ServerPlayer player) {
        player.level().playSound(null, player.blockPosition(), ModSounds.YUM_HEART_USE.get(), SoundSource.PLAYERS);
    }
