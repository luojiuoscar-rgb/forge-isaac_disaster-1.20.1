...
public class TheHierophantR extends TarotAbility {
    private static final CompositeTrigger TRIGGER = new CompositeTrigger(List.of(
            new SimpleTrigger(ModTriggerTypes.EMTPY, ModExecutableEffects.THE_HIEROPHANT_R)
    ));

    public TheHierophantR() {
        super(TRIGGER);
    }
    ...
    public void makeSound(ServerPlayer player) {
        ...
        player.playNotifySound(ModSounds.THE_HIEROPHANT_R.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    }
    ...
    public List<Component> getDesc() {
        ...
        if (ClientDataManager.getInstance().getCountFromId(ItemId.TAROT_CLOTH.getId()) > 0){
            description.add(DescriptionHelper.getSynergyDesc(
                    ...
                    Component.translatable("item.isaac_disaster.action.give_bone_heart", 3)
            ));
        }
