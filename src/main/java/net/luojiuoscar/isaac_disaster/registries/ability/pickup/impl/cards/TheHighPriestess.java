...
public class TheHighPriestess extends TarotAbility {
    private static final CompositeTrigger TRIGGER = new CompositeTrigger(List.of(
            new SimpleTrigger(ModTriggerTypes.EMTPY, ModExecutableEffects.THE_HIGH_PRIESTESS)
    ));
    ...
    public List<Component> getDesc() {
        ...
        if (ClientDataManager.getInstance().getCountFromId(ItemId.TAROT_CLOTH.getId()) > 0){
            description.add(DescriptionHelper.getSynergyDesc(
                    ...
                    Component.translatable("item.isaac_disaster.the_high_priestess.synergy.lore.1")
            ));
        }
