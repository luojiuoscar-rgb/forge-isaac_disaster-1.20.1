...
public class RangeDown extends PillEffect implements AddDamageIfLowQuality {

    @Override
    public PillEffect redirect(ServerPlayer player) {
        return PlayerHelper.getPillQuality(player) > 0 ? (PillEffect) ModExecutableEffects.RANGE_UP.get() : this;
    }

    @Override
    protected boolean pillActive(ServerPlayer player, boolean isHorse, ExecutableEffectContext context) {
        StatManager.RANGE.apply(player, isHorse ? -0.8 : -0.4);
        return true;
    }
