...
public class HealthDown extends PillEffect implements AddDamageIfLowQuality {
    @Override
    public PillEffect redirect(ServerPlayer player) {
        if (PlayerHelper.getPillQuality(player) > 0){
            return (PillEffect) ModExecutableEffects.HEALTH_UP.get();

        }else if (player.getMaxHealth() <= StatManager.MAX_HEALTH.getBonus()) {
            if (PlayerHelper.getPillQuality(player) < 0){
                return (PillEffect) ModExecutableEffects.I_FOUND_PILLS.get();
            }else{
                return (PillEffect) ModExecutableEffects.HEALTH_UP.get();
            }
        }
    ...
    public void makeSound(ServerPlayer player) {
        player.playNotifySound(ModSounds.HEALTH_DOWN.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
        player.playNotifySound(ModSounds.PILL_USE_BAD.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    }

    @Override
    public void makeSoundH(ServerPlayer player) {
        player.playNotifySound(ModSounds.HEALTH_DOWN_H.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
