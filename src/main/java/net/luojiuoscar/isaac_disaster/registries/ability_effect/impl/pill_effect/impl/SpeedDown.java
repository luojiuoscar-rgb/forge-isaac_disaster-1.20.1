...
public class SpeedDown extends PillEffect implements AddDamageIfLowQuality {

    @Override
    public PillEffect redirect(ServerPlayer player) {
        return PlayerHelper.getPillQuality(player) > 0 ? (PillEffect) ModExecutableEffects.SPEED_UP.get() : this;
    }

    @Override
    protected boolean pillActive(ServerPlayer player, boolean isHorse, ExecutableEffectContext context) {
        StatManager.MOVEMENT_SPEED.apply(player, isHorse ? -1.2 : -.6);
        return true;
    }
