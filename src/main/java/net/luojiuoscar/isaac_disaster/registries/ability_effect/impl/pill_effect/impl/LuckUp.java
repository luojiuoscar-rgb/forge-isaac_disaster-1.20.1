...
public class LuckUp extends PillEffect {

    @Override
    public PillEffect redirect(ServerPlayer player) {
        return PlayerHelper.getPillQuality(player) < 0 ? (PillEffect) ModExecutableEffects.LUCK_DOWN.get() : this;
    }
    ...
    public void makeSound(ServerPlayer player) {
        player.playNotifySound(ModSounds.LUCK_UP.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
        player.playNotifySound(ModSounds.PILL_USE_GOOD_WITH_EFFECT.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    }

    @Override
    public void makeSoundH(ServerPlayer player) {
        player.playNotifySound(ModSounds.LUCK_UP_H.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
